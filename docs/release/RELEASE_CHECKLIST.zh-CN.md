...
## 最终 v1.0.0 发布处置 — 2026-09-20

**决定：** 发布批准人明确接受下列例外后，发布 `v1.0.0`。该决定不改写本文档中历史 RC1 发现，也不会把缺失证据标记为 PASS。

| 最终门禁项 | 处置 | 证据 / 限制 |
| --- | --- | --- |
| RC BLOCKER/HIGH 工程缺陷 | 已关闭 | `a641ac7` 修复 RC 标识和测试计划覆盖；`3114b65` 修复摘要报告、筛选、预览/证据和 PDF KPI 定义问题，并加入回归测试。 |
| 自动化工程验证 | PASS | 最终版本元数据变更后，Ruff 通过；140 项测试按受限分组全部通过。 |
| UAT | 已接受例外 | 发布批准人称 UAT 已完成，但仓库中没有 UAT 计划、结果、场景证据或签署产物；本审计无法独立核验关键 UAT 用例。 |
| 产品负责人决策 BA-01–BA-08 | 已接受例外 | 没有带日期的决策记录。候选 KPI/异常/数据契约口径仍仅限合成演示。 |
| 性能目标 | 已接受例外 | 未记录已批准的负载、硬件边界或目标。 |
| 下方历史 RC1 表 | 历史记录 | F01、F06–F12 的仪表盘/报表失败早于 `3114b65`；其余 FAIL/NOT TESTED 行仍是证据缺口或草案治理发现，除非另行解决。 |

**发布范围：** 仅限明确标识为合成数据的本地单用户使用。不得用于生产控制、合规决策、公开托管或客户机密数据。
